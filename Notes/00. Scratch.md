...
## Unicode
- Unicode is an encoding standard that uses up to 32 bits or more as required to capture all known languages and characters
- Unicode uses two encoding forms: 8-bit and 16-bit based on the data being encoded
- Unicode Emojis contain the color and combination of emojis is the result of combining the Unicode characters of various emojis, although emojis might look a bit different on Android, Apple, etc the underlying Unicode is the same
- Context matters: We follow various methods to present Information in various forms like text, emoji, and colors for example, each pixel can be 8 bits to represent the combination of RGB, music is made up of notes and loudness, video is moving images
